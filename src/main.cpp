...
String formatCSV(const String &date, int tMed, float turbidity) { // These -> will have to be manually expanded for now
    String csv_data =
        "Date: " + date + "," +
...
// old school JSON //
String formatJSON(const String &date, int tMed, float turbidity) { // These -> will have to be manually expanded for now
    String json_data = String::format(
        "{\"date\":%s,\"lat\":%f,\"lng\":%f,\"speed\":%f,\"course\":%f,\"tdsMedian\":%f,\"turbidity\":%f}",
        date.c_str(),
...
void loop() {
   ...
   if(millis()-printTimepoint > logInterval)
   {
      ...
      String ldata = formatCSV(date, tdsValue, turbidityValue);
      logToSD(ldata);
      #endif
      
      #ifdef DEBUGGING // SERIAL
      //String sdata = formatJSON(date, tdsMedian, turbidityValue);
      String sdata = formatCSV(date, tdsValue, turbidityValue);
